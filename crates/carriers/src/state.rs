...
pub struct AppState {
    ...
    /// Sieve moderation policies: the built-ins plus everything discovered under `sieve_scripts`.
...
impl AppState {
    ...
    pub async fn load(config: Config, authenticator: MessageAuthenticator) -> Result<Self> {
        ...
        info!(
            ...
            global_before = policy.global_before_count(),
            global_after = policy.global_after_count(),
            domains = policy.domains().count(),
            "loaded policies"
        );
...
/// Compile the policy engine named by `config`: the built-ins, plus everything discovered under
/// the single `sieve_scripts` root (moderation policies + global/domain drop-ins) — see
/// [`PolicyEngine::load_root`].
pub fn load_policy_engine(config: &Config) -> Result<PolicyEngine> {
    match &config.sieve_scripts {
        Some(dir) => PolicyEngine::load_root(dir)
            .with_context(|| format!("loading Sieve scripts from {}", dir.display())),
